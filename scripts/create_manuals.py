...
def add_revision_and_toc(doc, entries):
    ...
    add_matrix(doc, ["版", "日付", "更新内容", "更新者"], [
        ["1.1", TODAY, "最終レビュー：権限・未実装範囲・引継ぎ確認を明確化", "一般社団法人 玉島青年会議所"],
...
ADMIN_CHAPTERS = [
    ...
    (8, "出欠管理", "事業担当・事務局", "期限前、事業後", "回答状況を確認し、必要な催促と記録を行う。", [
        ...
        ("訂正", ["本人以外の回答変更は、本人確認または担当者の根拠を残す。", "CSVダウンロードは現時点で画面上のUIのみで、実ファイル出力は未実装。運用記録は画面確認または別途承認した手段で扱う。"]),
    ...
    (9, "お知らせ管理", "理事長・専務・事務局", "連絡事項発生時", "対象者に必要な連絡を、重要度と公開期間を含めて発信する。", [
        ...
        ("編集・削除", ["誤記訂正は更新日時が残ることを前提に行う。", "公開終了後の扱いと削除可否は年度内のルールに従う。", "お知らせの保存・公開範囲設定は実装済み。対象者ごとの通知レコード自動作成と外部プッシュ配信は未実装。"]),
    ...
    (10, "資料管理", "事務局・各委員会担当", "資料配布時", "年度・イベント・公開範囲に沿って資料を整理する。", [
        ...
        ("取扱い", ["個人情報・未公開資料は公開範囲を限定する。", "差し替え時は旧版の扱いを記録する。", "資料画面の登録・ダウンロードは現時点でUIのみ。メタデータ保存、Supabase Storage保存、実ファイルダウンロードは未実装のため、正式資料の唯一の保管先として使わない。"]),
    ...
    (11, "日常運用", "事務局・年度管理担当", "毎日・毎週・毎月", "未対応事項をためず、年度情報を最新に保つ。", [
        ...
        ("毎週・毎月", ["期限超過出欠、会員ステータス、予定変更、資料公開範囲を確認する。", "月次で現在年度の管理者一覧と監査ログを確認する。資料保存・CSV・通知配信の未実装範囲は、代替運用の担当と保管場所を記録する。"]),
...
YEAR_CHAPTERS = [
    ...
    (6, "管理者権限の引継ぎ", "現年度・次年度理事長、専務", "切替直前", "新年度の管理者だけが適切に管理できる状態にする。", [
        ("確認", ["次年度の admin / president / secretary を有効にする。", "履歴保持のため旧年度の所属記録は削除しない。ただし、次年度で不要な人に admin / president / secretary を付与していないことを、現在年度の所属一覧で確認する。", "旧年度の役職が有効でも現在年度ではないため管理権限にならないこと、管理者と一般会員の両方で確認する。"]),
...
SYSTEM_CHAPTERS = [
    ...
    (7, "バックアップ", "システム管理者", "月次・変更前", "ソースとデータを復元できる状態にする。", [
        ("対象", ["GitHubのmainとリリース履歴。", "Supabase Databaseのエクスポートまたは、契約プランで利用できるバックアップ。", "Vercel・Supabase・GitHubの権限一覧と、重要な設定変更の記録。"]),
        ("運用", ["月次バックアップの実施日・担当者・保管場所を記録する。", "Supabase Dashboardでバックアップ機能と復元可能な範囲を契約プランごとに確認する。", "年1回以上、復元手順を別環境で確認する。認証情報やSecretをバックアップ記録へ書かない。"]),
...
def build_checklists():
    ...
    add_checklist_page(doc, "3. 年度引継ぎチェックリスト（切替・確認）", [
        "新年度を作成し、日付を確認した", "前年度コピー後の内容を修正した", "新年度の管理者を有効化した", "現在年度の管理者一覧に旧年度のみの担当者が残っていないことを確認した", "旧年度の履歴を削除していないことを確認した", "管理者・一般会員で権限を確認した", "スマートフォンで主要画面を確認した", "引継ぎ完了日と未解決事項を記録した",
    ], "確認担当: ____________________", "年度: ____________________")
    doc.add_page_break()
    ...
    add_matrix(doc, ["手順", "実施者", "確認"], [
        ...
        ["3. 初期情報を本人へ案内", "管理者", "安全な伝達。初期情報は画面を閉じた後に再表示不可"],
    ...
    add_matrix(doc, ["手順", "実施者", "確認"], [
        ...
        ["3. お知らせ・リマインド", "担当者", "お知らせ保存は可能。対象者への通知自動作成・外部配信は未実装のため、当面は別途周知する"],
...
def main():
    ...
    build_manual(
        ...
        [
            ...
            ("付録B 引継ぎ時の権限テスト", ["現在年度の管理者で会員・年度を管理できる", "旧年度のみの担当者が管理操作できない", "一般会員で管理操作が拒否される", "未ログインで保護画面がログインへ移動する", "スマートフォン表示を確認した"]),
